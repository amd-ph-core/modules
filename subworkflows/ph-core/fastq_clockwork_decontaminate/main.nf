...
include { CLOCKWORK_MINIMAP2      } from '../../../modules/ph-core/clockwork/minimap2/main'
include { CLOCKWORK_REMOVECONTAM } from '../../../modules/ph-core/clockwork/removecontam/main'

workflow FASTQ_CLOCKWORK_DECONTAMINATE {
    take:
    ch_reads         // channel (mandatory): [ val(meta), [ path(reads) ] ]
    ch_reference     // channel (mandatory): [ path(reference) ]
    ch_ref_metadata  // channel (mandatory): [ path(ref_metadata) ]
    ...
    emit:
    counts               = CLOCKWORK_REMOVECONTAM.out.counts
    decontaminated_fastq = CLOCKWORK_REMOVECONTAM.out.decontaminated_fastq
    contaminant_fastq    = CLOCKWORK_REMOVECONTAM.out.contaminant_fastq
